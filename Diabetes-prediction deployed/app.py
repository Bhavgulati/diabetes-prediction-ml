...
logger = logging.getLogger('diabetesai')

def log_prediction(user_id, user_email, inputs, risk_percent, risk_category, elapsed_ms):
    logger.info(
        f"PREDICTION | user={user_email} | "
        f"glucose={inputs.get('glucose')} bmi={inputs.get('bmi')} age={inputs.get('age')} | "
        f"risk={risk_percent}% ({risk_category}) | "
        f"time={elapsed_ms}ms"
    )

def log_api_call(endpoint, ip, status, elapsed_ms):
    logger.info(
        f"API_CALL | endpoint={endpoint} | ip={ip} | "
        f"status={status} | time={elapsed_ms}ms"
    )

def log_error(endpoint, error, user_email=None):
    logger.error(
        f"ERROR | endpoint={endpoint} | "
        f"user={user_email or 'anonymous'} | error={str(error)}"
    )
...
def load_model_version(version, filename):
    ...
    if os.path.exists(path):
        ...
        logger.info(f"MODEL_LOADED | version={version} | accuracy={acc}%")
        return True
    return False
...
def generate_pdf_report(user, risk_percent, risk_category, risk_color, shap_data, inputs):
    ...
    header_title = ParagraphStyle('ht', fontSize=11, fontName='Helvetica-Bold', textColor=WHITE, leading=14)
    section_head = ParagraphStyle('sh', fontSize=10, fontName='Helvetica-Bold', textColor=WHITE, leading=13)
    body_sm      = ParagraphStyle('bs', fontSize=8.5, fontName='Helvetica', textColor=DARK_GRAY, leading=13)
    disclaimer_s = ParagraphStyle('ds', fontSize=7.5, fontName='Helvetica', textColor=HexColor('#7f8c8d'), leading=11, alignment=TA_CENTER)
    label_s      = ParagraphStyle('ls', fontSize=7.5, fontName='Helvetica-Bold', textColor=MID_GRAY, leading=10, spaceAfter=1)
    value_s      = ParagraphStyle('vs', fontSize=9, fontName='Helvetica-Bold', textColor=DARK_GRAY, leading=11)
    ...
    header_data = [[
        ...
        Paragraph(f'<b>DIABETES RISK ASSESSMENT REPORT</b><br/>Report ID: {report_id}<br/>Generated: {now_str}',
                  ParagraphStyle('hr', fontSize=8, fontName='Helvetica', textColor=WHITE, leading=12, alignment=TA_RIGHT))
    ]]
    header_table = Table(header_data, colWidths=[W*0.5, W*0.5])
    header_table.setStyle(TableStyle([
        ('BACKGROUND',(0,0),(-1,-1),NAVY),('TOPPADDING',(0,0),(-1,-1),14),
        ('BOTTOMPADDING',(0,0),(-1,-1),14),('LEFTPADDING',(0,0),(0,-1),14),
        ('RIGHTPADDING',(-1,0),(-1,-1),14),('VALIGN',(0,0),(-1,-1),'MIDDLE'),
    ]))
    story.append(header_table)

    stripe = Table([['','','']], colWidths=[W*0.33,W*0.34,W*0.33])
    stripe.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),RISK_COLOR),
    ...
    patient_data = [
        ...
        [
            ...
            Table([
                [Paragraph(f'{risk_percent}%', ParagraphStyle('rp',fontSize=42,fontName='Helvetica-Bold',textColor=RISK_COLOR,leading=46,alignment=TA_CENTER))],
                [Paragraph(f'{risk_category.upper()} RISK', ParagraphStyle('rc',fontSize=13,fontName='Helvetica-Bold',textColor=RISK_COLOR,leading=16,alignment=TA_CENTER))],
                [Paragraph('See a Doctor Immediately' if risk_category=='High' else 'Monitor Health Closely' if risk_category=='Moderate' else 'Maintain Healthy Habits',
                            ParagraphStyle('rr',fontSize=8,fontName='Helvetica',textColor=DARK_GRAY,leading=11,alignment=TA_CENTER))],
            ], colWidths=[W*0.46],
               style=[('ALIGN',(0,0),(-1,-1),'CENTER'),('TOPPADDING',(0,0),(-1,-1),4),
                      ('BOTTOMPADDING',(0,0),(-1,-1),4),('BOX',(0,0),(-1,-1),1.5,RISK_COLOR),
    ...
    outer.setStyle(TableStyle([
        ...
        ('VALIGN',(0,0),(-1,-1),'TOP'),('BACKGROUND',(0,0),(-1,-1),LIGHT_GRAY),
    ...
    def section_header(title, icon=''):
        t = Table([[Paragraph(f'{icon}  {title}', section_head)]], colWidths=[W])
        t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),MID_BLUE),('TOPPADDING',(0,0),(-1,-1),7),
    ...
    def status_label(metric, value):
        ...
        try:
            ...
            for lo, hi, label in ranges:
                if lo <= v <= hi:
                    colors_map = {'NORMAL':GREEN,'PRE-DIABETIC':AMBER,'HIGH':RED,'ELEVATED':AMBER,'OVERWEIGHT':AMBER,'OBESE':RED,'LOW':AMBER}
                    return label, colors_map.get(label, MID_GRAY)
        except:
            pass
        return '', MID_GRAY
    ...
    for metric, val, ref, key in bio_rows:
        s_result = status_label(key, val) if key else ('', MID_GRAY)
        if s_result and s_result[0]:
            s_text, s_color = s_result
            status_cell = Paragraph(f'<b>{s_text}</b>', ParagraphStyle('st',fontSize=7.5,fontName='Helvetica-Bold',textColor=s_color,alignment=TA_CENTER))
        else:
            status_cell = Paragraph('—', ParagraphStyle('st',fontSize=8,fontName='Helvetica',textColor=MID_GRAY,alignment=TA_CENTER))
        table_data.append([
            Paragraph(metric, body_sm),
            Paragraph(f'<b>{val}</b>', ParagraphStyle('bv',fontSize=9,fontName='Helvetica-Bold',textColor=DARK_GRAY,alignment=TA_CENTER)),
            Paragraph(ref, ParagraphStyle('br',fontSize=8.5,fontName='Helvetica',textColor=HexColor('#7f8c8d'),alignment=TA_CENTER)),
            status_cell
        ])
    bio_table = Table(table_data, colWidths=[W*0.32,W*0.22,W*0.26,W*0.20])
    bio_table.setStyle(TableStyle([
        ('BACKGROUND',(0,0),(-1,0),NAVY),('TOPPADDING',(0,0),(-1,-1),7),
        ('BOTTOMPADDING',(0,0),(-1,-1),7),('LEFTPADDING',(0,0),(-1,-1),8),
        ('RIGHTPADDING',(0,0),(-1,-1),8),('ROWBACKGROUNDS',(0,1),(-1,-1),[WHITE,LIGHT_GRAY]),
        ('GRID',(0,0),(-1,-1),0.4,HexColor('#dce1e7')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),
    ...
    story.append(Spacer(1,4))
    story.append(Paragraph('The following analysis shows how each biomarker contributed to your personalised risk prediction:',
        ParagraphStyle('note',fontSize=8,fontName='Helvetica-Oblique',textColor=HexColor('#7f8c8d'),leading=11,spaceAfter=6)))
    shap_headers_row = [
    ...
    shap_rows_data = [shap_headers_row]
    for item in shap_data[:6]:
        ec = RED if item['direction']=='increases' else GREEN
        shap_rows_data.append([
            Paragraph(item['feature'], body_sm),
            Paragraph(str(item['value']), ParagraphStyle('sv',fontSize=9,fontName='Helvetica-Bold',textColor=DARK_GRAY,alignment=TA_CENTER)),
            Paragraph(f"{abs(item['shap'])}%", ParagraphStyle('sc',fontSize=9,fontName='Helvetica-Bold',textColor=ec,alignment=TA_CENTER)),
            Paragraph('▲ INCREASES RISK' if item['direction']=='increases' else '▼ REDUCES RISK',
                      ParagraphStyle('se',fontSize=7.5,fontName='Helvetica-Bold',textColor=ec,alignment=TA_CENTER)),
        ])
    shap_table = Table(shap_rows_data, colWidths=[W*0.30,W*0.20,W*0.22,W*0.28])
    ...
    if risk_category == 'High':
        urgency_color = RED
        urgency_text  = '⚠️  URGENT: This report indicates HIGH diabetes risk. Immediate medical consultation is strongly advised.'
        recs = [
        ...
        urgency_color = AMBER
        urgency_text  = '⚡  ADVISORY: Elevated diabetes risk detected. Medical review recommended within 4–6 weeks.'
        recs = [
        ...
        urgency_color = GREEN
        urgency_text  = '✅  LOW RISK: No immediate concerns. Maintain healthy habits and schedule annual screening.'
        recs = [
    ...
    urgency = Table([[Paragraph(urgency_text, ParagraphStyle('urg',fontSize=9,fontName='Helvetica-Bold',textColor=WHITE,leading=13))]], colWidths=[W])
    urgency.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),urgency_color),('TOPPADDING',(0,0),(-1,-1),10),('BOTTOMPADDING',(0,0),(-1,-1),10),('LEFTPADDING',(0,0),(-1,-1),12)]))
    story.append(urgency)
    story.append(Spacer(1,6))

    cat_colors = {'IMMEDIATE':RED,'MEDICAL':MID_BLUE,'DIETARY':GREEN,'LIFESTYLE':HexColor('#1a5276'),'MONITORING':HexColor('#6c3483'),'PREVENTIVE':HexColor('#1a5276')}
    rec_rows = []
    for cat, text in recs:
        rec_rows.append([Paragraph(f'<b>{cat}</b>', ParagraphStyle('rc',fontSize=7,fontName='Helvetica-Bold',textColor=WHITE,alignment=TA_CENTER)), Paragraph(text, body_sm)])
    rec_styles = [('TOPPADDING',(0,0),(-1,-1),6),('BOTTOMPADDING',(0,0),(-1,-1),6),('LEFTPADDING',(0,0),(-1,-1),8),('RIGHTPADDING',(0,0),(-1,-1),8),('GRID',(0,0),(-1,-1),0.4,HexColor('#dce1e7')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('ROWBACKGROUNDS',(0,0),(-1,-1),[WHITE,LIGHT_GRAY])]
    for i,(cat,_) in enumerate(recs):
        rec_styles.append(('BACKGROUND',(0,i),(0,i),cat_colors.get(cat,NAVY)))
    ...
    footer = Table([[Paragraph(
        f'<b>MEDICAL DISCLAIMER:</b> This report is generated by DiabetesAI for informational and screening purposes only. '
        f'It does NOT constitute a medical diagnosis, clinical opinion, or professional medical advice. '
    ...
    footer.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),LIGHT_GRAY),('TOPPADDING',(0,0),(-1,-1),10),('BOTTOMPADDING',(0,0),(-1,-1),10),('LEFTPADDING',(0,0),(-1,-1),12),('RIGHTPADDING',(0,0),(-1,-1),12),('BOX',(0,0),(-1,-1),0.5,HexColor('#dce1e7'))]))
    story.append(footer)
    doc.build(story)
    buffer.seek(0)
    return buffer
...
def predict():
    ...
    if request.method == 'POST':
        ...
        preg=int(raw['pregnancies']); glucose=int(raw['glucose']); bp=int(raw['bloodpressure'])
        ...
        proba        = active_model.predict_proba(data)[0]
        risk_percent = round(proba[1]*100,1)

        if risk_percent>=70:   risk_category,risk_color='High','#ff4d6d'
        elif risk_percent>=40: risk_category,risk_color='Moderate','#f59e0b'
        else:                  risk_category,risk_color='Low','#10b981'
        ...
        if isinstance(shap_values,list): shap_vals=np.array(shap_values[1][0]).flatten()
        elif hasattr(shap_values,'values'): shap_vals=np.array(shap_values.values[0]).flatten()
        else: shap_vals=np.array(shap_values[0]).flatten()

        shap_data = sorted([{
            'feature':FEATURE_NAMES[i],'value':round(float(data[0][i]),2),
        ...
        pred_record = Prediction(user_id=current_user.id, risk_percent=risk_percent,
            risk_category=risk_category, prediction=prediction, pregnancies=preg,
            glucose=glucose, bloodpressure=bp, skinthickness=st, insulin=insulin,
        ...
        try: send_email_report(current_user.email, current_user.name, risk_percent, risk_category)
        except: pass

        inputs = {'pregnancies':preg,'glucose':glucose,'bloodpressure':bp,
                  'skinthickness':st,'insulin':insulin,'bmi':bmi,'dpf':dpf,'age':age}

        return render_template('result.html', user=current_user, prediction=prediction,
            risk_percent=risk_percent, risk_category=risk_category, risk_color=risk_color,
            feature_importance=json.dumps(feature_importance), shap_data=shap_data,
            model_comparison=json.dumps(model_comparison), inputs=json.dumps(inputs),
            google_maps_key=GOOGLE_MAPS_KEY, model_version=used_version)
...
def download_report():
    ...
    data   = request.json
    buffer = generate_pdf_report(user=current_user, risk_percent=data.get('risk_percent'),
        risk_category=data.get('risk_category'), risk_color=data.get('risk_color','#00d4ff'),
        shap_data=data.get('shap_data',[]), inputs=data.get('inputs',{}))
...
def api_predict():
    ...
    try:
        arr = np.array([[float(data['pregnancies']),float(data['glucose']),float(data['bloodpressure']),
                         float(data['skinthickness']),float(data['insulin']),float(data['bmi']),
                         float(data['dpf']),float(data['age'])]])
        proba         = active_model.predict_proba(arr)[0]
        risk_percent  = round(proba[1]*100,1)
        risk_category = 'High' if risk_percent>=70 else 'Moderate' if risk_percent>=40 else 'Low'

        elapsed = round((time.time()-start_time)*1000,1)
        log_api_call('/api/predict', request.remote_addr, 200, elapsed)
...
def api_history():
    predictions = Prediction.query.filter_by(user_id=current_user.id).order_by(Prediction.created_at.asc()).all()
    history = [{'date':p.created_at.strftime('%d %b %Y'),'risk':p.risk_percent,'category':p.risk_category} for p in predictions]
